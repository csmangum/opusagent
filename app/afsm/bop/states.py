...
Each state implements the AFSMState interface and defines its processing logic and allowed transitions.
"""
from typing import Tuple, Optional, Dict, Any, List
from ..states.base import AFSMState, StateContext, StateTransition

class IdleState(AFSMState):
    """Initial idle state when no interaction is happening."""
    
    def __init__(self):
        transitions = [
            StateTransition(target_state="greeting", condition="User initiates conversation")
        ]
        super().__init__(
            name="idle",
...
class GreetingState(AFSMState):
    ...
    def __init__(self):
        transitions = [
            StateTransition(target_state="authenticating", condition="Authentication needed")
        ]
        super().__init__(
            name="greeting",
    ...
    async def process(self, input_text: str, context: StateContext) -> Tuple[str, Optional[str], Dict[str, Any]]:
        ...
        return response, "authenticating", {"conversation_history": context.conversation_history}


class AuthenticatingState(AFSMState):
    """Handles user authentication."""
    
    def __init__(self):
        transitions = [
            StateTransition(target_state="authentication_failed", condition="Authentication failed"),
            StateTransition(target_state="main_menu", condition="Authentication successful")
        ]
        super().__init__(
            name="authenticating",
    ...
    async def process(self, input_text: str, context: StateContext) -> Tuple[str, Optional[str], Dict[str, Any]]:
        ...
        if (("account" in input_lower or "number" in input_lower) and 
            ("pin" in input_lower or "password" in input_lower) and
            any(c.isdigit() for c in input_lower)):
            is_authenticated = True
        
        if is_authenticated:
            self.write_to_scratchpad("Authentication successful. Transitioning to main menu.")
            
            response = "Authentication successful. Welcome to your Bank of Peril account."
            ...
            self.write_to_scratchpad("Authentication failed. Transitioning to authentication_failed state.")
            ...
            return response, "authentication_failed", {"conversation_history": context.conversation_history}


class AuthenticationFailedState(AFSMState):
    """Handles failed authentication attempts."""
    
    def __init__(self):
        transitions = [
            StateTransition(target_state="authenticating", condition="Retry authentication"),
    ...
    async def process(self, input_text: str, context: StateContext) -> Tuple[str, Optional[str], Dict[str, Any]]:
        ...
        if any(term in input_lower for term in ["retry", "try again", "account", "pin", "number"]):
            ...
            return response, "ending_interaction", {"conversation_history": context.conversation_history}


class MainMenuState(AFSMState):
    """Main menu state after successful authentication."""
    
    def __init__(self):
        transitions = [
            StateTransition(target_state="identify_intent", condition="User makes request"),
            StateTransition(target_state="ending_interaction", condition="User wants to exit")
        ]
        super().__init__(
            name="main_menu",
    ...
    async def process(self, input_text: str, context: StateContext) -> Tuple[str, Optional[str], Dict[str, Any]]:
        ...
        if any(term in input_lower for term in ["exit", "quit", "log out", "logout", "sign out"]):
        ...
        self.write_to_scratchpad("User has made a request. Transitioning to identify intent.")
        
        # No response here as the identify_intent state will generate the response
        return "", "identify_intent", {"conversation_history": context.conversation_history}


class IdentifyIntentState(AFSMState):
    """Identifies user intent from their request."""
    
    def __init__(self):
        transitions = [
            StateTransition(target_state="gather_balance_details", condition="Balance inquiry"),
            StateTransition(target_state="gather_transfer_details", condition="Transfer request"),
            StateTransition(target_state="gather_bill_pay_details", condition="Bill payment request"),
            StateTransition(target_state="clarification_needed", condition="Intent unclear"),
    ...
    async def process(self, input_text: str, context: StateContext) -> Tuple[str, Optional[str], Dict[str, Any]]:
        ...
        if any(term in input_lower for term in ["balance", "how much", "account"]):
        ...
        elif any(term in input_lower for term in ["transfer", "send money", "move money"]):
            ...
            return response, "clarification_needed", {"conversation_history": context.conversation_history}


class GatherBalanceDetailsState(AFSMState):
    """Gathers details needed for a balance inquiry."""
    
    def __init__(self):
        transitions = [
            StateTransition(target_state="confirm_action", condition="Details collected"),
    ...
    async def process(self, input_text: str, context: StateContext) -> Tuple[str, Optional[str], Dict[str, Any]]:
        ...
        if account_type:
            self.write_to_scratchpad(f"User selected {account_type} account. All details collected.")
...
class GatherTransferDetailsState(AFSMState):
    ...
    def __init__(self):
        transitions = [
            StateTransition(target_state="confirm_action", condition="Details collected"),
            StateTransition(target_state="main_menu", condition="Cancel operation"),
            StateTransition(target_state="clarification_needed", condition="Information missing or ambiguous")
        ]
        super().__init__(
    ...
    async def process(self, input_text: str, context: StateContext) -> Tuple[str, Optional[str], Dict[str, Any]]:
        ...
        if "source_account" not in context.metadata:
        ...
        elif "source_account" in context.metadata and "destination_account" not in context.metadata:
            ...
            if amount_match:
                ...
                self.write_to_scratchpad("All transfer details collected. Moving to confirmation.")
...
class GatherBillPayDetailsState(AFSMState):
    ...
    async def process(self, input_text: str, context: StateContext) -> Tuple[str, Optional[str], Dict[str, Any]]:
        ...
        if "payee" not in context.metadata:
            ...
            if amount_match:
                ...
                self.write_to_scratchpad("All bill payment details collected. Moving to confirmation.")
...
class ConfirmActionState(AFSMState):
    """Asks for confirmation before executing a banking transaction."""
    
    def __init__(self):
        transitions = [
            StateTransition(target_state="executing_transaction", condition="User confirms"),
    ...
    async def process(self, input_text: str, context: StateContext) -> Tuple[str, Optional[str], Dict[str, Any]]:
        ...
        if not input_text:
            ...
            return confirmation_message, None, {"conversation_history": context.conversation_history}
        ...
        if any(term in input_lower for term in ["yes", "yeah", "proceed", "confirm", "ok", "sure"]):
            self.write_to_scratchpad("User confirmed the action. Proceeding to execute transaction.")
            return "", "executing_transaction", {"conversation_history": context.conversation_history}
            
        else:
            self.write_to_scratchpad("User cancelled the action. Returning to main menu.")
            ...
            return response, "main_menu", {"conversation_history": context.conversation_history}


class ExecutingTransactionState(AFSMState):
    """Executes the banking transaction."""
    
    def __init__(self):
        transitions = [
            StateTransition(target_state="reporting_success", condition="Transaction successful"),
            StateTransition(target_state="reporting_failure", condition="Transaction failed"),
...
class ReportingSuccessState(AFSMState):
    """Reports successful transaction to the user."""
    
    def __init__(self):
        transitions = [
            StateTransition(target_state="main_menu", condition="Return to main menu"),
            StateTransition(target_state="ending_interaction", condition="End interaction")
        ]
        super().__init__(
    ...
    async def process(self, input_text: str, context: StateContext) -> Tuple[str, Optional[str], Dict[str, Any]]:
        ...
        if intent == "balance_inquiry":
            ...
            success_message = f"Successfully transferred ${amount:.2f} to {destination}."
        ...
        if not input_text:
            return success_message, None, {"conversation_history": context.conversation_history}
        ...
        if any(term in input_lower for term in ["exit", "quit", "log out", "logout", "no", "bye"]):
            return "", "ending_interaction", {"conversation_history": context.conversation_history}
        else:
            return "", "main_menu", {"conversation_history": context.conversation_history}


class ReportingFailureState(AFSMState):
    ...
    async def process(self, input_text: str, context: StateContext) -> Tuple[str, Optional[str], Dict[str, Any]]:
        ...
        if not input_text:
            return failure_message, None, {"conversation_history": context.conversation_history}
        ...
        if any(term in input_lower for term in ["exit", "quit", "log out", "logout", "end"]):
            return "", "ending_interaction", {"conversation_history": context.conversation_history}
        else:
            return "", "main_menu", {"conversation_history": context.conversation_history}


class ClarificationNeededState(AFSMState):
    """Handles unclear user requests by asking for clarification."""
    
    def __init__(self):
        transitions = [
            StateTransition(target_state="identify_intent", condition="Clear intent provided"),
            StateTransition(target_state="main_menu", condition="Return to main menu")
        ]
        super().__init__(
    ...
    async def process(self, input_text: str, context: StateContext) -> Tuple[str, Optional[str], Dict[str, Any]]:
        ...
        if any(term in input_lower for term in ["balance", "check"]):
            ...
            self.write_to_scratchpad("Still unclear what user wants. Asking again with more specific options.")
...
class HandlingErrorState(AFSMState):
    ...
    async def process(self, input_text: str, context: StateContext) -> Tuple[str, Optional[str], Dict[str, Any]]:
        ...
        if not input_text:
            ...
            return error_message, None, {"conversation_history": context.conversation_history}
        ...
        if any(term in input_lower for term in ["main", "menu", "continue"]):
            ...
            response = "I understand. Ending this session. Thank you for using Bank of Peril."
...
class EndingInteractionState(AFSMState):
    """Final state for ending the interaction."""
    
    def __init__(self):
        transitions = [
            StateTransition(target_state="idle", condition="New session")
        ]
        super().__init__(
