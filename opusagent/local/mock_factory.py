...
def create_customer_service_mock(
    audio_dir: str = "opusagent/local/audio_mocks",
...
def create_sales_mock(
    audio_dir: str = "demo/audio", logger: Optional[logging.Logger] = None
) -> LocalRealtimeClient:
    """
...
def create_simple_mock(
    ...
    for key, text in responses.items():
        ...
        response_configs[key] = LocalResponseConfig(
            text=text, audio_file=audio_file, delay_seconds=0.03, audio_chunk_delay=0.2
        )
...
def create_function_testing_mock(
    ...
    response_configs = {
        ...
        "account_function": LocalResponseConfig(
            ...
            function_call={
                "name": "get_account_balance",
                "arguments": {"account_id": "12345", "include_transactions": True},
            },
            delay_seconds=0.03,
        ),
    }
...
def create_audio_testing_mock(
    audio_files: Dict[str, str], logger: Optional[logging.Logger] = None
) -> LocalRealtimeClient:
    """
    ...
    default_config = LocalResponseConfig(
        text="Default audio response", delay_seconds=0.03, audio_chunk_delay=0.2
    )
