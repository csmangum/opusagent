#!/usr/bin/env python3
"""
Transcription Capability Validation Script for LocalRealtimeClient

This script comprehensively validates the transcription capabilities of the
LocalRealtimeClient, testing both PocketSphinx and Whisper backends with
various audio scenarios and configurations.

Features:
- Tests both PocketSphinx and Whisper transcription backends
...
Usage:
    python scripts/validate_transcription_capability.py [options]

Options:
    --backend pocketsphinx|whisper|both    Test specific backend(s)
    --audio-file path/to/audio.wav        Use specific audio file for testing
    --generate-test-audio                 Generate test audio files
...
class TranscriptionValidator:
    """Comprehensive validator for transcription capabilities."""
    ...
    async def test_error_handling(self, backend: str) -> bool:
        """Test error handling with invalid audio data."""
        test_name = f"Error Handling: {backend}"
        ...
        try:
            ...
            transcriber.start_session()
            result = await transcriber.transcribe_chunk(invalid_audio)
            transcriber.end_session()
            await transcriber.cleanup()
            
            # Should handle gracefully without crashing
            self.log_test_result(test_name, True, "Handled invalid audio gracefully")
            return True
            
        except Exception as e:
            self.log_test_result(test_name, False, f"Error handling failed: {e}")
            return False
    ...
    async def run_validation(self, backends: List[str], audio_file: Optional[str] = None, 
                           generate_test_audio: bool = False, output_dir: str = "validation_results") -> Dict[str, Any]:
        """Run comprehensive transcription validation."""
        self.logger.info("Starting transcription capability validation...")
        ...
        if generate_test_audio:
            test_audio_file = await self.generate_test_audio(output_dir)
        ...
        for backend in backends:
            ...
            # Test error handling
            await self.test_error_handling(backend)
        
        # Save results
        results_file = os.path.join(output_dir, f"transcription_validation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def setup_logging(verbose: bool = False) -> logging.Logger:
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler('transcription_validation.log')
        ]
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Validate transcription capability of LocalRealtimeClient")
    parser.add_argument("--backend", choices=["pocketsphinx", "whisper", "both"], 
                       default="both", help="Test specific backend(s)")
