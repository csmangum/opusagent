...
from opusagent.models.tool_models import OpenAITool, ToolParameter, ToolParameters
...
try:
    ...
    logger.warning(
        "Audio playback not available. Install sounddevice and other audio dependencies."
    )
...
def _get_audio_manager():
    ...
    if _global_audio_manager is None and AUDIO_PLAYBACK_AVAILABLE and AudioManager:
        try:
            # Use the existing AudioManager from audiocodes for file processing
            _global_audio_manager = AudioManager(logger=logger)
            logger.info("Global audio manager initialized for file processing")
...
class PlayAudioTool(OpenAITool):
    ...
    name: str = "play_audio"
    description: str = (
        "Play a specified audio file to respond to the user. Use this to 'speak' by selecting appropriate audio files."
    )
...
async def func_play_audio(arguments: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            # Use the AudioManager to load and chunk the audio file
            audio_chunks = audio_manager.load_audio_chunks(str(audio_path))
            logger.info(f"Loaded {len(audio_chunks)} audio chunks for {filename}")
        except Exception as e:
        ...
        # In a real implementation, this would interface with a playback system
        logger.info(f"Would play audio file: {filename} ({len(audio_chunks)} chunks)")
        ...
        return {
            ...
            "context": context,
            "function_name": "play_audio",
            "message": f"Audio file loaded successfully: {filename}",
            "chunks_loaded": len(audio_chunks),
            "note": "Audio playback not yet implemented - file loaded successfully",
        }
...
class TextAudioAgent:
    ...
    def _initialize_handlers(self):
        ...
        logger.info(
            f"Function handler initialized with audio directory: {self.audio_directory}"
        )
        ...
        try:
            ...
            if event_type == "response.function_call_arguments.delta":
                if self.function_handler:
                    await self.function_handler.handle_function_call_arguments_delta(
                        data
                    )
                ...
                if self.function_handler:
                    ...
                    await self.function_handler.handle_function_call_arguments_done(
                        data
                    )
