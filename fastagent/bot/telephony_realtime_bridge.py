"""
Bridge module for connecting telephony WebSocket protocol with OpenAI Realtime API.

This module provides a robust integration between telephony WebSocket protocols
...
class TelephonyRealtimeBridge:
    """
    Bridge between telephony WebSocket protocols and OpenAI Realtime API.

    This class implements a sophisticated bridge that handles:
    1. Real-time audio streaming between telephony platforms and OpenAI
    ...
    async def create_client(
        self, conversation_id: str, websocket: WebSocket, model: str = DEFAULT_MODEL
    ) -> None:
        """
    ...
    async def send_audio_chunk(self, conversation_id: str, audio_chunk: str) -> None:
        """
        Process and forward an audio chunk from telephony client to OpenAI.
        ...
        try:
            ...
            self.audio_latencies[conversation_id] = (
    ...
    async def _handle_openai_responses(self, conversation_id: str) -> None:
        """
        Continuously process and forward audio responses from OpenAI to telephony client.
        ...
        if not client or not websocket:
            logger.warning(
                f"Missing client or websocket for conversation: {conversation_id}"
            )
            return
        ...
        try:
            ...
            base_message = {"type": "playStream.chunk", "streamId": str(stream_id)}
            ...
            logger.info(
                f"Response handling task cancelled for conversation: {conversation_id}"
            )
        except Exception as e:
    ...
    async def _cleanup_stream(self, conversation_id: str, stream_id: int) -> None:
        ...
        websocket = self.websockets.get(conversation_id)
        if websocket:
            stop_message = {"type": "playStream.stop", "streamId": str(stream_id)}
            try:
    ...
    async def close_client(self, conversation_id: str) -> None:
        ...
        if client:
            try:
                await client.close()
                logger.info(
                    f"Closed OpenAI Realtime client for conversation: {conversation_id}"
                )
            except Exception as e:
                logger.error(
                    f"Error closing client for conversation {conversation_id}: {str(e)}"
                )
