...
@pytest.fixture
def audio_handler(
    mock_telephony_websocket, mock_realtime_websocket, mock_call_recorder
):
...
async def test_handle_incoming_audio_success(audio_handler):
    ...
    )  # Should be 4800 bytes (resampled from 16kHz to 24kHz for OpenAI)
...
async def test_handle_outgoing_audio_success(audio_handler):
    ...
    audio_handler.call_recorder.record_bot_audio.assert_called_once_with(
        TEST_AUDIO_DELTA
    )
...
async def test_get_audio_stats(audio_handler):
    ...
    # The duration calculation in get_audio_stats uses 24kHz for OpenAI
    # 4800 bytes = 100ms at 24kHz 16-bit
