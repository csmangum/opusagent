...
class AudioConfig:
    ...
    chunk_size: int = 3200  # 200ms at 16kHz 16-bit
    chunk_size_large: int = 32000  # 2 seconds at 16kHz 16-bit
    format: str = "raw/lpcm16"
    supported_formats: List[str] = field(default_factory=lambda: ["raw/lpcm16", "g711/ulaw", "g711/alaw"])
...
class VADConfig:
    ...
    speech_start_threshold: int = 2
    speech_stop_threshold: int = 3
    device: str = "cpu"
    chunk_size: int = 512
...
class StaticDataConfig:
    ...
    phrases_mapping_file: Path = field(default_factory=lambda: Path("opusagent/local/audio/phrases_mapping.yml"))
...
class ApplicationConfig:
    ...
    def validate(self) -> List[str]:
        ...
        if self.transcription.confidence_threshold < 0 or self.transcription.confidence_threshold > 1:
