...
class AudioCodesLocalRealtimeValidator:
    ...
    def _setup_logger(self) -> logging.Logger:
        ...
        if not logger.handlers:
            ...
            log_file = (
                logs_dir / f"audiocodes_local_realtime_validation_{timestamp}.log"
            )
    ...
    def _generate_test_audio(self) -> List[str]:
        ...
        try:
            ...
            if audio_data is None:
                self.logger.warning(
                    "No real audio files found, falling back to synthetic audio"
                )
            ...
            self.logger.info(
                f"Generated {len(chunks)} audio chunks from real audio file"
            )
            return chunks
    ...
    def _load_audio_file(self, file_path: Path) -> bytes:
        ...
        try:
            ...
            with wave.open(str(file_path), "rb") as wav_file:
                ...
                self.logger.debug(
                    f"Audio file info: {frames} frames, {sample_width} bytes/sample, "
                ...
                if sample_width == 1:
                    ...
                    audio_array = (
                        audio_array.astype(np.int16) - 128
                    )  # Convert to signed
                    ...
                    audio_array = (
                        audio_24bit[:, 1].astype(np.int16) << 8
                    ) | audio_24bit[:, 2].astype(np.int16)
                elif sample_width == 4:
    ...
    def _resample_audio(
        self, audio_array: Any, original_rate: int, target_rate: int
    ) -> Any:
    ...
    def _generate_synthetic_audio(self) -> List[str]:
        ...
        try:
            ...
            audio_signal = (
                audio_signal / np.max(np.abs(audio_signal)) * 0.7
            )  # Leave some headroom
    ...
    async def test_session_initiation(self) -> bool:
        ...
        try:
            ...
            response = await self.wait_for_message_type(
                TelephonyEventType.SESSION_ACCEPTED
            )
            if not response:
                self.logger.error("No session.accepted received")
                return False

            if not self.validate_message_structure(
                response, TelephonyEventType.SESSION_ACCEPTED
            ):
                return False
    ...
    async def test_audio_streaming(self) -> bool:
        ...
        try:
            ...
            response = await self.wait_for_message_type(
                TelephonyEventType.USER_STREAM_STARTED
            )
            if not response:
            ...
            if not self.validate_message_structure(
                response, TelephonyEventType.USER_STREAM_STARTED
            ):
                return False
            ...
            response = await self.wait_for_message_type(
                TelephonyEventType.USER_STREAM_STOPPED
            )
            if not response:
            ...
            if not self.validate_message_structure(
                response, TelephonyEventType.USER_STREAM_STOPPED
            ):
                return False
    ...
    async def test_local_realtime_response(self) -> bool:
        ...
        try:
            ...
            play_stream_start = await self.wait_for_message_type(
                TelephonyEventType.PLAY_STREAM_START, timeout=15.0
            )
            ...
            if not self.validate_message_structure(
                play_stream_start, TelephonyEventType.PLAY_STREAM_START
            ):
                return False
            ...
            while audio_chunks_received < max_chunks:
                ...
                if message.get("type") == TelephonyEventType.PLAY_STREAM_CHUNK:
                    if not self.validate_message_structure(
                        message, TelephonyEventType.PLAY_STREAM_CHUNK
                    ):
                        return False
                    ...
                    if not self.validate_message_structure(
                        message, TelephonyEventType.PLAY_STREAM_STOP
                    ):
                        return False
                    self.play_stream_active = False
            ...
            if audio_chunks_received > 0:
                self.logger.info(
                    f"✓ Received {audio_chunks_received} audio chunks from local realtime"
                )
                self.validation_results["tests_passed"] += 1
                return True
            else:
    ...
    async def test_vad_integration(self) -> bool:
        ...
        try:
            ...
            speech_started_events = [
                e
                for e in vad_events
                if e.get("type") == TelephonyEventType.USER_STREAM_SPEECH_STARTED
            ]
            speech_stopped_events = [
                e
                for e in vad_events
                if e.get("type") == TelephonyEventType.USER_STREAM_SPEECH_STOPPED
            ]
            speech_committed_events = [
                e
                for e in vad_events
                if e.get("type") == TelephonyEventType.USER_STREAM_SPEECH_COMMITTED
            ]
            ...
            if (
                speech_started_events
                and speech_stopped_events
                and speech_committed_events
            ):
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        report = {
            "validation_summary": {
                ...
                "total_tests": self.validation_results["tests_passed"]
            ...
            "message_statistics": {
                ...
                "local_realtime_events": len(
                    self.validation_results["local_realtime_events"]
                ),
            },
    ...
    def print_summary(self):
        ...
        report_file = (
            Path("logs")
            / f"audiocodes_local_realtime_validation_report_{timestamp}.json"
        )
...
async def run_validation_tests(
    validator: AudioCodesLocalRealtimeValidator, test_name: Optional[str] = None
) -> bool:
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Validate AudioCodes Bridge with Local Realtime"
    )
    parser.add_argument(
        "--server-url",
    ...
    parser.add_argument(
        "--bot-name", default="LocalRealtimeBot", help="Bot name for testing"
    )
    ...
    if not os.getenv("USE_LOCAL_REALTIME", "false").lower() in (
        "true",
        "1",
        "yes",
        "on",
        ...
        print(
            "Run with: USE_LOCAL_REALTIME=true python scripts/validate_audiocodes_local_realtime.py"
        )
        return False
