...
async def test_handle_user_stream_start():
    ...
    with patch('opusagent.handlers.stream_handlers.handle_user_stream_start') as mock_handler:
        # Create expected response
        mock_response = {"type": "userStream.started", "conversationId": "test-id"}
        mock_handler.return_value = mock_response
        
        # Call handler
        from opusagent.handlers.stream_handlers import handle_user_stream_start
...
async def test_handle_user_stream_chunk():
    ...
    # Test with patched handlers
    with patch('opusagent.handlers.stream_handlers.handle_user_stream_chunk') as mock_handler:
        mock_handler.return_value = None  # No response for chunk messages
        
        # Call handler
        from opusagent.handlers.stream_handlers import handle_user_stream_chunk
...
async def test_handle_session_resume():
    ...
    # Test with patched handlers
    with patch('opusagent.handlers.session_handlers.handle_session_resume') as mock_handler:
        mock_response = {"type": "session.resumed", "conversationId": "test-id"}
        mock_handler.return_value = mock_response
        
        # Call handler
        from opusagent.handlers.session_handlers import handle_session_resume
...
async def test_handle_activities():
    ...
    # Test with patched handlers
    with patch('opusagent.handlers.activity_handlers.handle_activities') as mock_handler:
        mock_response = {"type": "activities.processed", "conversationId": "test-id"}
        mock_handler.return_value = mock_response
        
        # Call handler
        from opusagent.handlers.activity_handlers import handle_activities
...
async def test_multiple_messages_handling():
    ...
    with patch('opusagent.websocket_manager.handle_user_stream_chunk', AsyncMock(return_value=None)) as mock_chunk_handler:
        # Run test
        await websocket_manager.handle_websocket(websocket)
