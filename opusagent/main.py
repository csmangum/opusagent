...
USE_LOCAL_REALTIME = os.getenv("USE_LOCAL_REALTIME", "false").lower() in (
    "true",
    "1",
    "yes",
    "on",
...
LOCAL_REALTIME_CONFIG = {
    "enable_transcription": os.getenv(
        "LOCAL_REALTIME_ENABLE_TRANSCRIPTION", "true"
    ).lower()
    in ("true", "1", "yes", "on"),
    "setup_smart_responses": os.getenv(
        "LOCAL_REALTIME_SETUP_SMART_RESPONSES", "true"
    ).lower()
    in ("true", "1", "yes", "on"),
    "vad_config": {
...
async def root():
    ...
    return {
        ...
        "configuration": {
            ...
            "local_realtime_config": (
                LOCAL_REALTIME_CONFIG if USE_LOCAL_REALTIME else None
            ),
        },
        "endpoints": {
