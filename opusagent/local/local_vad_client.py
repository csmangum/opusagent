...
class LocalVADClient:
        ...
        try:
            ...
            async for message in self._ws:
                try:
                    ...
                    self.logger.warning(
                        f"[LOCAL VAD] Received non-JSON message: {message}"
                    )
    ...
    async def _process_bridge_message(self, data: Dict[str, Any]):
        ...
        if msg_type == "session.accepted":
            ...
            self.logger.info(
                f"[LOCAL VAD] Session accepted with format: {self.media_format}"
            )
            ...
            self.logger.info(
                f"[LOCAL VAD] Play stream started: {self.current_stream_id}"
            )
            ...
            if audio_chunk:
                ...
                try:
                    ...
                    self.logger.debug(
                        f"[LOCAL VAD] Received audio chunk ({len(audio_chunk)} bytes base64)"
                    )
                except Exception as e:
            ...
            self.play_stream_active = False
            self.logger.info(
                f"[LOCAL VAD] Play stream stopped: {self.current_stream_id}"
            )
    ...
    async def initiate_session(self, conversation_id: Optional[str] = None) -> bool:
        ...
        self.logger.info(
            f"[LOCAL VAD] Sent session.initiate for conversation: {self.conversation_id}"
        )
        ...
        try:
            ...
            def audio_callback(indata, frames, time_info, status):
                ...
                if self._detect_speech(audio_bytes):
                    if not self.speech_detected:
                        ...
                        if self.loop:
                            asyncio.run_coroutine_threadsafe(
                                self._start_user_stream(), self.loop
                            )
                        else:
                            self.logger.warning(
                                "[LOCAL VAD] No event loop set for run_coroutine_threadsafe (start_user_stream)"
                            )
                    ...
                    if self.loop:
                        asyncio.run_coroutine_threadsafe(
                            self._send_audio_chunk(audio_bytes), self.loop
                        )
                    else:
                        self.logger.warning(
                            "[LOCAL VAD] No event loop set for run_coroutine_threadsafe (send_audio_chunk)"
                        )
                else:
                    if self.speech_detected:
                        if self.silence_start_time is None:
                            ...
                            time.time() - self.silence_start_time
                            ...
                            if self.loop:
                                asyncio.run_coroutine_threadsafe(
                                    self._stop_user_stream(), self.loop
                                )
                            else:
                                self.logger.warning(
                                    "[LOCAL VAD] No event loop set for run_coroutine_threadsafe (stop_user_stream)"
                                )
    ...
    def _playback_loop(self):
        ...
        try:
            ...
            def playback_callback(outdata, frames, time, status):
                if status:
                    self.logger.warning(
                        f"[LOCAL VAD] Playback callback status: {status}"
                    )
    ...
    async def run_conversation(self, duration: float = 30.0):
        ...
        try:
            ...
            self.logger.info(
                f"[LOCAL VAD] Running conversation for {duration} seconds..."
            )
            self.logger.info(
                "[LOCAL VAD] Speak into your microphone to start the conversation"
            )
...
async def main():
    ...
    parser.add_argument(
        "--duration", type=float, default=30.0, help="Conversation duration in seconds"
    )
    parser.add_argument(
        "--vad-sensitivity", type=float, default=0.1, help="VAD sensitivity (0.0-1.0)"
    )
    parser.add_argument(
        "--vad-silence",
        type=float,
