        ...
        with Container(classes="main-interface", id="main-interface"):
            ...
            with Horizontal(classes="main-container"):
                ...
                with Vertical(classes="right-panel"):
                    ...
                    # Function calls
                    self.functions_panel = FunctionCallsPanel(classes="functions-section")
...
"""
    ...
    async def _set_review_session(self, session: ReviewSession) -> None:
        ...
        if self.status_bar:
            ...
            if session.metadata:
                self.status_bar.update_audio_status(f"Duration: {session.metadata.get_duration_str()}")
    ...
    async def _update_transcript_panel(self, session: ReviewSession) -> None:
        ...
        for entry in session.filtered_transcript:
            if entry.speaker == "user":
                ...
                self.transcript_panel.add_system_message(entry.text, entry.get_time_str())
