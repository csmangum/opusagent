...
class StateTransition(BaseModel):
    ...
    def __str__(self) -> str:
        return f"Transition to {self.target_state}" + (f" when {self.condition}" if self.condition else "")
...
class AFSMState:
    """Base class for all states in the Agentic Finite State Machine."""
    
    def __init__(self, name: str, description: str, allowed_transitions: List[StateTransition] = None):
        """
        ...
        self.scratchpad = ""
        self.logger = logging.getLogger(f"fastagent.states.{name}")
