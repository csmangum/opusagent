...
class ReasoningState(AFSMState, ScratchpadStateMixin):
    ...
    async def process(self, input_text: str, context: StateContext) -> Tuple[str, Optional[str], StateContext]:
        """
    ...
    def _determine_next_state(self, conclusions: List[str], context: StateContext) -> Optional[str]:
        """
...
class ProductRecommendationState(ReasoningState):
    ...
    def __init__(self):
        super().__init__(
            ...
            allowed_transitions=[
                StateTransition(target_state="checkout", condition="User wants to purchase"),
                StateTransition(target_state="product_details", condition="User wants more info")
            ]
        )
    ...
    def _form_hypotheses(self, facts: List[str], context: StateContext) -> List[str]:
        ...
        if "User is price sensitive" in facts and "User values quality" in facts:
            hypotheses.append("Mid-range products with good value proposition may be best")
    ...
    def _generate_response(self, conclusions: List[str], context: StateContext) -> str:
        ...
        specific_recs = [c for c in conclusions if c.startswith("Specifically recommend")]
        if specific_recs:
