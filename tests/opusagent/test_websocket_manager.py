...
class TestWebSocketManager:
    ...
    def mock_env_vars(self):
        """Mock environment variables required by WebSocketConfig."""
        with patch.dict(
            "os.environ",
            {"OPENAI_API_KEY": "test_api_key", "OPENAI_ORG_ID": "test_org_id"},
        ):
            yield
    ...
    def mock_websockets_connect(self, event_loop):
        ...
        with patch("opusagent.websocket_manager.websockets.connect") as mock_connect:
            ...
            mock_connect.side_effect = None  # Reset any side effects
            yield mock_connect
    ...
    async def manager(self, mock_config):
        ...
        yield manager
        # Cleanup health monitoring task
    ...
    async def test_get_connection_reuse_existing(
        self, manager, mock_websockets_connect
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_connection_at_max_connections(
        self, manager, mock_websockets_connect
    ):
    ...
    @pytest.mark.asyncio
    async def test_connection_context_manager_with_exception(
        self, manager, mock_websockets_connect
    ):
    ...
    @pytest.mark.asyncio
    async def test_cleanup_unhealthy_connections(
        self, manager, mock_websockets_connect
    ):
