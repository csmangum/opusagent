...
class AudioUtils:
    ...
    def load_wav_file(filepath: str) -> Tuple[bytes, int, int]:
        ...
        try:
            with wave.open(filepath, "rb") as wav_file:
                ...
                logger.info(
                    f"Loaded WAV: {filepath} - {sample_rate}Hz, {channels}ch, {sample_width*8}bit, {len(frames)} bytes"
                )
    ...
    @staticmethod
    def load_audio_file(
        filepath: str, target_sample_rate: int = 16000
    ) -> Tuple[bytes, int, int]:
        """
        ...
        if ADVANCED_AUDIO_AVAILABLE and librosa is not None and np is not None:
            try:
                # Load with librosa and convert to target sample rate
                audio_array, sample_rate = librosa.load(
                    filepath, sr=target_sample_rate, mono=True
                )
                ...
                logger.info(
                    f"Loaded audio: {filepath} - {sample_rate}Hz, 1ch, {len(audio_data)} bytes"
                )
                return audio_data, int(sample_rate), 1

            except Exception as e:
                logger.error(f"Error loading audio file {filepath} with librosa: {e}")
        else:
            logger.warning(
                "Advanced audio loading not available. Install librosa for MP3/FLAC support."
            )
    ...
    def chunk_audio_data(
        audio_data: bytes, chunk_size: int, overlap: int = 0
    ) -> List[bytes]:
        """
    ...
    def resample_audio(
        ...
        if not ADVANCED_AUDIO_AVAILABLE or np is None:
        ...
        try:
            ...
            if librosa is not None:
                resampled = librosa.resample(
                    audio_array, orig_sr=from_rate, target_sr=to_rate
                )
    ...
    def visualize_audio_level(
        audio_data: bytes, max_bars: int = 13, sample_width: int = 2
    ) -> str:
        """
    ...
    def create_waveform_visualization(
        audio_data: bytes, width: int = 50, height: int = 10, sample_width: int = 2
    ) -> List[str]:
        """
        ...
        """
        if not audio_data or not ADVANCED_AUDIO_AVAILABLE or np is None:
    ...
    def get_audio_duration(
        audio_data: bytes, sample_rate: int, channels: int = 1, sample_width: int = 2
    ) -> float:
        """
