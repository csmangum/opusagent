...
These tests verify the functionality of the TelephonyClient class,
...
from opusagent.services.websocket_client import TelephonyClient


@pytest.fixture
def audio_client():
    """Create an TelephonyClient instance for testing."""
    return TelephonyClient("wss://test-telecom.example.com/ws")
...
async def test_connect_failure(audio_client):
    ...
    with patch(
        "opusagent.services.websocket_client.websockets.connect",
...
async def test_initiate_session_success(audio_client):
    ...
    with patch("opusagent.services.websocket_client.uuid.uuid4", return_value=mock_uuid):
