...
async def test_handle_outgoing_audio_audiocodes_success(bridge, mock_websocket):
    """Test successful handling of outgoing audio with resampling."""
    # Set up bridge state
    bridge.conversation_id = "test-conv-123"
    ...
    # Mock AudioUtils.resample_audio
    with patch('opusagent.bridges.audiocodes_bridge.AudioUtils') as mock_audio_utils:
        # Create test audio data (24kHz PCM16)
        test_audio_24k = b'\x00\x00\x01\x00\x02\x00' * 1000  # 6000 bytes at 24kHz
        test_audio_16k = b'\x00\x00\x01\x00\x02\x00' * 667   # 4002 bytes at 16kHz (resampled)
        
        mock_audio_utils.resample_audio.return_value = test_audio_16k
        ...
        mock_audio_utils.resample_audio.assert_called_once_with(
            b'test audio data', 24000, 16000
        )
        ...
        # The audioChunk should be the base64 encoded resampled audio, not the original
        assert sent_payload["audioChunk"] == base64.b64encode(test_audio_16k).decode("utf-8")
...
async def test_handle_outgoing_audio_audiocodes_with_existing_stream(bridge, mock_websocket):
    ...
    # Mock AudioUtils.resample_audio
    with patch('opusagent.bridges.audiocodes_bridge.AudioUtils') as mock_audio_utils:
        test_audio_16k = b'\x00\x00\x01\x00\x02\x00' * 667
        mock_audio_utils.resample_audio.return_value = test_audio_16k
...
@pytest.mark.asyncio
async def test_handle_outgoing_audio_audiocodes_resampling_error(bridge, mock_websocket):
    """Test outgoing audio handling when resampling fails."""
    # Set up bridge state
    bridge.conversation_id = "test-conv-123"
    bridge._closed = False
    bridge.platform_websocket = mock_websocket
    ...
    # Mock AudioUtils.resample_audio to raise an exception
    with patch('opusagent.bridges.audiocodes_bridge.AudioUtils') as mock_audio_utils:
            mock_audio_utils.resample_audio.side_effect = Exception("Resampling failed")
...
async def test_handle_outgoing_audio_audiocodes_empty_audio(bridge, mock_websocket):
    ...
    # Mock AudioUtils.resample_audio
    with patch('opusagent.bridges.audiocodes_bridge.AudioUtils') as mock_audio_utils:
        mock_audio_utils.resample_audio.return_value = b''  # Empty resampled audio
        ...
        mock_audio_utils.resample_audio.assert_called_once()
...
@pytest.mark.asyncio
async def test_handle_outgoing_audio_audiocodes_resampling_parameters(bridge, mock_websocket):
    """Test that resampling is called with correct parameters."""
    # Set up bridge state
    bridge.conversation_id = "test-conv-123"
    bridge._closed = False
    bridge.platform_websocket = mock_websocket
    ...
    with patch('opusagent.bridges.audiocodes_bridge.AudioUtils') as mock_audio_utils:
        ...
        # Verify resampling was called with correct parameters
        mock_audio_utils.resample_audio.assert_called_once()
        call_args = mock_audio_utils.resample_audio.call_args
        
        # First argument should be the decoded audio bytes
        assert call_args[0][0] == b'test audio data'
        # Second argument should be source sample rate (24kHz)
        assert call_args[0][1] == 24000
        # Third argument should be target sample rate (16kHz)
...
async def test_handle_outgoing_audio_audiocodes_base64_encoding(bridge, mock_websocket):
    """Test that resampled audio is properly base64 encoded."""
    # Set up bridge state
    bridge.conversation_id = "test-conv-123"
    bridge._closed = False
    bridge.platform_websocket = mock_websocket
    ...
    # Mock AudioUtils.resample_audio
    with patch('opusagent.bridges.audiocodes_bridge.AudioUtils') as mock_audio_utils:
        # Create specific test audio that we can verify
        test_audio_16k = b'\x01\x00\x02\x00\x03\x00\x04\x00'
        mock_audio_utils.resample_audio.return_value = test_audio_16k
        ...
        # Verify the sent payload contains the correct base64 encoded audio
        sent_payload = mock_websocket.send_json.call_args[0][0]
        expected_base64 = "AQACAAMABAA="  # base64 encoding of b'\x01\x00\x02\x00\x03\x00\x04\x00'
        assert sent_payload["audioChunk"] == expected_base64
