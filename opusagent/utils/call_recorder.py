...
class CallRecorder:
    ...
    def __init__(
        ...
        self.target_sample_rate = (
            DEFAULT_SAMPLE_RATE  # Target rate for final recordings (for consistency)
        )
        self.channels = 1
        ...
        logger.info(
            f"Audio settings - Caller: {self.caller_sample_rate}Hz, Bot: {self.bot_sample_rate}Hz, Target: {self.target_sample_rate}Hz"
        )

        # Validate recording directory
        self._validate_recording_directory()

    def _validate_recording_directory(self):
        """Validate that the recording directory is accessible and writable."""
        try:
            ...
            if not os.access(self.recording_dir, os.W_OK):
                raise PermissionError(
                    f"Recording directory is not writable: {self.recording_dir}"
                )

            # Test file creation
            test_file = self.recording_dir / ".test_write"
            try:
                test_file.write_text("test")
                test_file.unlink()  # Clean up test file
            except Exception as e:
                raise PermissionError(f"Cannot write to recording directory: {e}")

            logger.debug(f"Recording directory validated: {self.recording_dir}")

        except Exception as e:
            logger.error(f"Recording directory validation failed: {e}")
            raise
            ...
            if ratio > 1:
                ...
                if filter_size > 1:
                    ...
                    filtered_audio = np.convolve(
                        audio_array, np.ones(filter_size) / filter_size, mode="same"
                    )
                else:
                ...
                old_indices = np.linspace(0, original_length - 1, new_length)
                resampled_audio = np.interp(
                    old_indices, np.arange(original_length), filtered_audio
                )
    ...
    def _init_wav_files(self):
        ...
        try:
            ...
            self.recording_dir.mkdir(parents=True, exist_ok=True)
            logger.debug(f"Recording directory ensured: {self.recording_dir}")
            ...
            try:
                ...
                self.caller_wav.setframerate(
                    self.target_sample_rate
                )  # Use target rate for consistency
                logger.debug(f"Caller WAV file initialized: {self.caller_audio_file}")
            except Exception as e:
                logger.error(f"Error initializing caller WAV file: {e}")
                raise
            ...
            try:
                ...
                self.bot_wav.setframerate(
                    self.target_sample_rate
                )  # Use target rate for consistency
                logger.debug(f"Bot WAV file initialized: {self.bot_audio_file}")
            except Exception as e:
                logger.error(f"Error initializing bot WAV file: {e}")
                raise
            ...
            try:
                ...
                self.stereo_wav.setframerate(
                    self.target_sample_rate
                )  # Use target rate for consistency
                logger.debug(f"Stereo WAV file initialized: {self.stereo_audio_file}")
            except Exception as e:
                logger.error(f"Error initializing stereo WAV file: {e}")
                raise

            logger.info("WAV files initialized for recording")

        except Exception as e:
            logger.error(f"Error initializing WAV files: {e}")
            logger.error(f"Recording directory: {self.recording_dir}")
            logger.error(f"Directory exists: {self.recording_dir.exists()}")
            logger.error(
                f"Directory writable: {os.access(self.recording_dir, os.W_OK) if self.recording_dir.exists() else 'N/A'}"
            )
            raise
    ...
    async def record_bot_audio(self, audio_chunk_b64: str) -> bool:
        ...
        try:
            ...
            if len(decoded_chunk) % 2 != 0:
                logger.warning(
                    f"Bot audio chunk has odd number of bytes: {len(decoded_chunk)}"
                )
    ...
    async def stop_recording(self) -> bool:
        ...
        try:
            ...
            # Close WAV files safely
            try:
                if self.caller_wav:
                    self.caller_wav.close()
                    self.caller_wav = None
                    logger.debug("Caller WAV file closed")
            except Exception as e:
                logger.error(f"Error closing caller WAV file: {e}")

            try:
                if self.bot_wav:
                    self.bot_wav.close()
                    self.bot_wav = None
                    logger.debug("Bot WAV file closed")
            except Exception as e:
                logger.error(f"Error closing bot WAV file: {e}")

            try:
                if self.stereo_wav:
                    self.stereo_wav.close()
                    self.stereo_wav = None
                    logger.debug("Stereo WAV file closed")
            except Exception as e:
                logger.error(f"Error closing stereo WAV file: {e}")

            # Save transcript and metadata with individual error handling
            save_errors = []
            ...
            try:
                ...
                logger.debug("Session events saved successfully")
            except Exception as e:
                error_msg = f"Error saving session events: {e}"
                logger.error(error_msg)
                save_errors.append(error_msg)
            ...
            try:
                ...
                logger.debug("Final stereo recording created successfully")
            except Exception as e:
                error_msg = f"Error creating final stereo recording: {e}"
                logger.error(error_msg)
                save_errors.append(error_msg)
            ...
            try:
                await self._log_session_event(
                    "recording_stopped",
                    {
                        ...
                        "total_audio_chunks": self.metadata.caller_audio_chunks
                        + self.metadata.bot_audio_chunks,
                    },
                )
                logger.debug("Recording stopped event logged")
            except Exception as e:
                error_msg = f"Error logging recording stopped event: {e}"
                logger.error(error_msg)
                save_errors.append(error_msg)
            ...
            if save_errors:
                logger.warning(
                    f"Recording stopped with {len(save_errors)} errors: {'; '.join(save_errors)}"
                )
                logger.info(
                    f"Recording stopped for conversation {self.conversation_id} (with warnings)"
                )
            else:
                logger.info(
                    f"Recording stopped and finalized for conversation {self.conversation_id}"
                )

            return True

        except Exception as e:
            logger.error(f"Critical error stopping recording: {e}")
            # Try to at least mark as finalized to prevent further issues
            self.finalized = True
            return False
    ...
    async def _save_transcript(self):
        ...
        try:
            transcript_data = {
                ...
                "end_time": (
                    self.metadata.end_time.isoformat()
        ...
        try:
            ...
            try:
                if self.caller_audio_buffer:
                    ...
                    logger.debug(
                        f"Caller audio buffer processed: {len(caller_audio)} samples"
                    )
            except Exception as e:
                logger.error(f"Error processing caller audio buffer: {e}")
                caller_audio = np.array([], dtype=np.int16)
            ...
            try:
                if self.bot_audio_buffer:
                    ...
                    logger.debug(
                        f"Bot audio buffer processed: {len(bot_audio)} samples"
                    )
            except Exception as e:
                logger.error(f"Error processing bot audio buffer: {e}")
                bot_audio = np.array([], dtype=np.int16)
            ...
            try:
                if len(caller_audio) < max_length:
                    ...
                    logger.debug(f"Caller audio padded to {len(caller_audio)} samples")
            except Exception as e:
                logger.error(f"Error padding caller audio: {e}")
                return
            ...
            try:
                if len(bot_audio) < max_length:
                    ...
                    logger.debug(f"Bot audio padded to {len(bot_audio)} samples")
            except Exception as e:
                logger.error(f"Error padding bot audio: {e}")
                return

            # Create stereo array (left=caller, right=bot)
            try:
                stereo_audio = np.column_stack((caller_audio, bot_audio))
                logger.debug(f"Stereo audio created: {stereo_audio.shape}")
            except Exception as e:
                logger.error(f"Error creating stereo array: {e}")
                return
            ...
            try:
                ...
                logger.info(
                    f"Final stereo recording created: {final_stereo_file} ({duration:.2f}s)"
                )
                logger.info(
                    f"Audio resampling applied - Bot audio resampled from {self.bot_sample_rate}Hz to {self.target_sample_rate}Hz"
                )

            except Exception as e:
                logger.error(f"Error writing final stereo recording to file: {e}")
                return

        except Exception as e:
            logger.error(f"Error creating final stereo recording: {e}")
            # Log additional context for debugging
            logger.error(f"Caller buffer chunks: {len(self.caller_audio_buffer)}")
            logger.error(f"Bot buffer chunks: {len(self.bot_audio_buffer)}")
            logger.error(f"Recording directory: {self.recording_dir}")
            logger.error(f"Target sample rate: {self.target_sample_rate}")
            logger.error(f"Sample width: {self.sample_width}")
    ...
    async def emergency_cleanup(self):
        """Emergency cleanup when recording fails catastrophically."""
        try:
            logger.warning(f"Emergency cleanup for conversation {self.conversation_id}")

            # Force close all WAV files
            for wav_file in [self.caller_wav, self.bot_wav, self.stereo_wav]:
                if wav_file:
                    try:
                        wav_file.close()
                        ...
                        logger.error(
                            f"Error closing WAV file during emergency cleanup: {e}"
                        )

            # Mark as finalized to prevent further issues
            self.recording_active = False
            self.finalized = True
            ...
            logger.info(f"Emergency cleanup completed for {self.conversation_id}")
    ...
    def get_recording_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "finalized": self.finalized,
            "recording_dir": str(self.recording_dir),
            "caller_audio_chunks": self.metadata.caller_audio_chunks,
            "bot_audio_chunks": self.metadata.bot_audio_chunks,
            "caller_audio_bytes": self.metadata.caller_audio_bytes,
            "bot_audio_bytes": self.metadata.bot_audio_bytes,
            "transcript_entries": self.metadata.transcript_entries,
            "caller_buffer_chunks": len(self.caller_audio_buffer),
            "bot_buffer_chunks": len(self.bot_audio_buffer),
            "wav_files_open": {
                "caller_wav": self.caller_wav is not None,
                "bot_wav": self.bot_wav is not None,
                "stereo_wav": self.stereo_wav is not None,
            },
        }

    @classmethod
    def test_sample_rate_detection(
        cls, audio_chunk_b64: str, test_rates: Optional[List[int]] = None
    ) -> dict:
        """
        ...
        try:
            ...
            for rate in test_rates:
                ...
                results[rate] = {
                    ...
                    "reasonable": 10
                    <= duration_ms
                    <= 500,  # Typical chunk should be 10-500ms
                }

            # Find most reasonable sample rates
            reasonable_rates = [
                rate for rate, info in results.items() if info["reasonable"]
            ]

            return {
                "chunk_info": {"bytes": len(decoded_chunk), "samples": samples},
