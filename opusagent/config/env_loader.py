...
from .models import (
    ApplicationConfig, ServerConfig, OpenAIConfig, AudioConfig, VADConfig,
    TranscriptionConfig, WebSocketConfig, QualityMonitoringConfig, LoggingConfig,
    MockConfig, TUIConfig, StaticDataConfig, SecurityConfig, Environment, LogLevel
)
...
def load_server_config() -> ServerConfig:
    ...
    environment = Environment.DEVELOPMENT if env_str == "development" else Environment.PRODUCTION
    if env_str == "testing":
...
def load_audio_config() -> AudioConfig:
    ...
    return AudioConfig(
        ...
        supported_formats=safe_convert(os.getenv("AUDIO_SUPPORTED_FORMATS"), List[str], ["raw/lpcm16", "g711/ulaw", "g711/alaw"]),
    )
...
def load_vad_config() -> VADConfig:
    ...
    return VADConfig(
        ...
        confidence_threshold=safe_convert(os.getenv("VAD_CONFIDENCE_THRESHOLD"), float, 0.5),
        silence_threshold=safe_convert(os.getenv("VAD_SILENCE_THRESHOLD"), float, 0.6),
        min_speech_duration_ms=safe_convert(os.getenv("VAD_MIN_SPEECH_DURATION_MS"), int, 500),
        speech_start_threshold=safe_convert(os.getenv("VAD_SPEECH_START_THRESHOLD"), int, 2),
        speech_stop_threshold=safe_convert(os.getenv("VAD_SPEECH_STOP_THRESHOLD"), int, 3),
        device=os.getenv("VAD_DEVICE", "cpu"),
        chunk_size=safe_convert(os.getenv("VAD_CHUNK_SIZE"), int, 512),
        confidence_history_size=safe_convert(os.getenv("VAD_CONFIDENCE_HISTORY_SIZE"), int, 5),
        force_stop_timeout_ms=safe_convert(os.getenv("VAD_FORCE_STOP_TIMEOUT_MS"), int, 2000),
...
def load_transcription_config() -> TranscriptionConfig:
    ...
    return TranscriptionConfig(
        ...
        chunk_duration=safe_convert(os.getenv("TRANSCRIPTION_CHUNK_DURATION"), float, 1.0),
        confidence_threshold=safe_convert(os.getenv("TRANSCRIPTION_CONFIDENCE_THRESHOLD"), float, 0.5),
        ...
        pocketsphinx_auto_resample=safe_convert(os.getenv("POCKETSPHINX_AUTO_RESAMPLE"), bool, True),
        pocketsphinx_input_sample_rate=safe_convert(os.getenv("POCKETSPHINX_INPUT_SAMPLE_RATE"), int, 24000),
...
def load_websocket_config() -> WebSocketConfig:
    ...
    return WebSocketConfig(
        ...
        max_connection_age=safe_convert(os.getenv("WEBSOCKET_MAX_CONNECTION_AGE"), float, 3600.0),
        max_idle_time=safe_convert(os.getenv("WEBSOCKET_MAX_IDLE_TIME"), float, 300.0),
        health_check_interval=safe_convert(os.getenv("WEBSOCKET_HEALTH_CHECK_INTERVAL"), float, 30.0),
        max_sessions_per_connection=safe_convert(os.getenv("WEBSOCKET_MAX_SESSIONS_PER_CONNECTION"), int, 10),
...
def load_quality_config() -> QualityMonitoringConfig:
    ...
    return QualityMonitoringConfig(
        ...
        max_clipping_percent=safe_convert(os.getenv("QUALITY_MAX_CLIPPING_PERCENT"), float, 0.1),
        ...
        enable_realtime_logging=safe_convert(os.getenv("QUALITY_ENABLE_REALTIME_LOGGING"), bool, True),
        enable_summary_reports=safe_convert(os.getenv("QUALITY_ENABLE_SUMMARY_REPORTS"), bool, True),
        summary_interval_seconds=safe_convert(os.getenv("QUALITY_SUMMARY_INTERVAL"), int, 60),
    )
...
def load_mock_config() -> MockConfig:
    ...
    return MockConfig(
        ...
        enable_transcription=safe_convert(os.getenv("LOCAL_REALTIME_ENABLE_TRANSCRIPTION"), bool, True),
        setup_smart_responses=safe_convert(os.getenv("LOCAL_REALTIME_SETUP_SMART_RESPONSES"), bool, True),
    )
...
def load_tui_config() -> TUIConfig:
    ...
    return TUIConfig(
        ...
        enable_audio_recording=safe_convert(os.getenv("TUI_ENABLE_RECORDING"), bool, True),
        recordings_dir=os.getenv("TUI_RECORDINGS_DIR", "test_logs"),
        max_recording_duration=safe_convert(os.getenv("TUI_MAX_RECORDING_DURATION"), int, 300),
        ...
        transcript_max_lines=safe_convert(os.getenv("TUI_TRANSCRIPT_MAX_LINES"), int, 500),
        ...
        filter_heartbeat_messages=safe_convert(os.getenv("TUI_FILTER_HEARTBEAT"), bool, True),
        ...
        auto_export_on_session_end=safe_convert(os.getenv("TUI_AUTO_EXPORT"), bool, False),
...
def load_static_data_config() -> StaticDataConfig:
    ...
    return StaticDataConfig(
        ...
        phrases_mapping_file=Path(os.getenv("PHRASES_MAPPING_FILE", "opusagent/local/audio/phrases_mapping.yml")),
...
def load_security_config() -> SecurityConfig:
    ...
    return SecurityConfig(
        ...
        rate_limiting_enabled=safe_convert(os.getenv("RATE_LIMITING_ENABLED"), bool, True),
        max_requests_per_minute=safe_convert(os.getenv("MAX_REQUESTS_PER_MINUTE"), int, 100),
...
def get_environment_info() -> Dict[str, Any]:
    ...
    return {
        "environment_variables_loaded": len([k for k in os.environ.keys() if k.startswith(('OPENAI_', 'VAD_', 'TUI_', 'WEBSOCKET_', 'LOG_'))]),
